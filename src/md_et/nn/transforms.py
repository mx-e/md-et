...
from md_et.nn.augmentation import get_random_reflections, get_random_rotations
...
def center_positions_on_center_of_mass(mol) -> dict:
    masses = th.tensor(atomic_masses[mol[Props.atomic_numbers]])
...
def require_grad_positions(batch) -> dict:
    batch[Props.positions].requires_grad = True
    return batch
