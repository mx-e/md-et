...
import torch as th
from md_et.nn.transforms import add_default_charge, add_default_multiplicity
...
def get_loaders(
    ...
    needed_props = {
        ...
        if prop in dataset_splits.dataset_props
    }
    if len(missing_props) > 0:
        logger.warning(
            f"Props not present in the dataset that are needed by the model {missing_props}"
        )
        ...
        if missing_props:
            raise ValueError(
                f"Missing needed props that cannot be computed or replaced by defaults: {missing_props}"
            )
